...
// bDotN) and grpmn (the singular part of the system-matrix kernel).
...
class SingularIntegralsOperator {
   public:
    ...
    // Test/expert entry points mirroring vmecpp's prepareUpdate/performUpdate
    ...
    void prepareUpdate(const T* d_a,
                       const T* d_b2,
                       const T* d_c,
                       const T* d_A,
                       const T* d_B2,
